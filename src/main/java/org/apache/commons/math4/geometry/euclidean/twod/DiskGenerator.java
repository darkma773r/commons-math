 ...
 * @since 3.3
 */
public class DiskGenerator implements SupportBallGenerator<Euclidean2D, Point2D> {

    /** {@inheritDoc} */
    @Override
    public EnclosingBall<Euclidean2D, Point2D> ballOnSupport(final List<Point2D> support) {
        ...
        if (support.size() < 1) {
            ...
            if (support.size() < 2) {
                ...
                if (support.size() < 3) {
                    return new EnclosingBall<>(new Point2D(0.5, vA, 0.5, vB),
                                                                    0.5 * vA.distance(vB),
                                                                    vA, vB);
                } else {
