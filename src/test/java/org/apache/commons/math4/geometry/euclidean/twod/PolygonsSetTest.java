...
public class PolygonsSetTest {
    ...
    public void testFull() {
        ...
        GeometryTestUtils.assertVectorEquals(Point2D.NaN, (Point2D) poly.getBarycenter(), TEST_TOLERANCE);
        ...
        for (double y = -1; y < 1; y += 0.1) {
            for (double x = -1; x < 1; x += 0.1) {
                GeometryTestUtils.assertNegativeInfinity(poly.projectToBoundary(new Point2D(x, y)).getOffset());
            }
        }
    }
    ...
    public void testEmpty() {
        ...
        for (double y = -1; y < 1; y += 0.1) {
            for (double x = -1; x < 1; x += 0.1) {
                GeometryTestUtils.assertPositiveInfinity(poly.projectToBoundary(new Point2D(x, y)).getOffset());
            }
        }
    }
    ...
    public void testBox() {
        ...
        GeometryTestUtils.assertVectorEquals(new Point2D(1, 0), (Point2D) box.getBarycenter(), TEST_TOLERANCE);
    ...
    public void testInvertedBox() {
        ...
        Assert.assertFalse(box.isEmpty());
        Assert.assertFalse(box.isFull());
        GeometryTestUtils.assertVectorEquals(Point2D.NaN, (Point2D) box.getBarycenter(), TEST_TOLERANCE);
    ...
    public void testHole() {
        ...
        for (double x = -0.999; x < 3.999; x += 0.11) {
            ...
            Point2D p = (Point2D) projection.getProjected();
            if (x < -0.5) {
    ...
    public void testLineIntersection() {
        ...
        // assert
        Line l1 = new Line(new Point2D(-1.5, 0.0), FastMath.PI / 4, TEST_TOLERANCE);
    ...
    public void testInconsistentHyperplanes() {
        ...
        new RegionFactory<Euclidean2D>().buildConvex(new Line(new Point2D(0, 0), new Point2D(0, 1), tolerance),
    ...
    private SubHyperplane<Euclidean2D> buildLine(Point2D start, Point2D end) {
    ...
    private SubHyperplane<Euclidean2D> buildHalfLine(Point2D start, Point2D end,
    ...
    private SubHyperplane<Euclidean2D> buildSegment(Point2D start, Point2D end) {
        Line   line  = new Line(start, end, TEST_TOLERANCE);
