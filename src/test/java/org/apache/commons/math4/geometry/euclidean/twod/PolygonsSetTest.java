...
public class PolygonsSetTest {
    ...
    public void testFull() {
        ...
        GeometryTestUtils.assertVectorEquals(Vector2D.NaN, (Vector2D) poly.getBarycenter(), TEST_TOLERANCE);
        ...
        for (double y = -1; y < 1; y += 0.1) {
            for (double x = -1; x < 1; x += 0.1) {
                GeometryTestUtils.assertNegativeInfinity(poly.projectToBoundary(new Vector2D(x, y)).getOffset());
            }
        }
    }
    ...
    public void testEmpty() {
        ...
        for (double y = -1; y < 1; y += 0.1) {
            for (double x = -1; x < 1; x += 0.1) {
                GeometryTestUtils.assertPositiveInfinity(poly.projectToBoundary(new Vector2D(x, y)).getOffset());
            }
        }
    }
    ...
    public void testInfiniteLines_single() {
        // arrange
        Line line = new Line(new Vector2D(0, 0), new Vector2D(1, 1), TEST_TOLERANCE);
        ...
        checkPoints(Region.Location.OUTSIDE, poly,
                new Vector2D(1, -1),
                new Vector2D(Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY));
        checkPoints(Region.Location.INSIDE, poly,
                new Vector2D(-1, 1),
                new Vector2D(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY));
    ...
    public void testInfiniteLines_twoIntersecting() {
        // arrange
        Line line1 = new Line(new Vector2D(0, 0), new Vector2D(1, 1), TEST_TOLERANCE);
        Line line2 = new Line(new Vector2D(1, -1), new Vector2D(0, 0), TEST_TOLERANCE);
        ...
        checkVertexLoopsEquivalent(new Vector2D[][] {
            {
                null,
                line2.toSpace(new Vector1D(-Float.MAX_VALUE)),
                line2.toSpace(new Vector1D(Float.MAX_VALUE))
            }
        }, poly.getVertices());
        ...
        checkPoints(Region.Location.INSIDE, poly,
                new Vector2D(-1, 0),
                new Vector2D(-Float.MAX_VALUE, Float.MAX_VALUE / 2.0));
        checkPoints(Region.Location.OUTSIDE, poly,
                new Vector2D(1, 0),
                new Vector2D(Float.MAX_VALUE, Float.MAX_VALUE / 2.0));
        checkPoints(Region.Location.BOUNDARY, poly, Vector2D.ZERO);
    ...
    public void testInfiniteLines_twoParallel_facingIn() {
        // arrange
        Line line1 = new Line(new Vector2D(1, 1), new Vector2D(0, 1), TEST_TOLERANCE);
        Line line2 = new Line(new Vector2D(0, -1), new Vector2D(1, -1), TEST_TOLERANCE);
        ...
        checkVertexLoopsEquivalent(new Vector2D[][] {
            {
                null,
                line1.toSpace(new Vector1D(-Float.MAX_VALUE)),
                line1.toSpace(new Vector1D(Float.MAX_VALUE))
            },
            {
                null,
    ...
    public void testInfiniteLines_twoParallel_facingOut() {
        // arrange
        Line line1 = new Line(new Vector2D(0, 1), new Vector2D(1, 1), TEST_TOLERANCE);
        Line line2 = new Line(new Vector2D(1, -1), new Vector2D(0, -1), TEST_TOLERANCE);
    ...
    public void testMixedFiniteAndInfiniteLines_explicitInfiniteBoundaries() {
        // arrange
        Line line1 = new Line(new Vector2D(3, 3), new Vector2D(0, 3), TEST_TOLERANCE);
        Line line2 = new Line(new Vector2D(0, -3), new Vector2D(3, -3), TEST_TOLERANCE);

        List<SubHyperplane<Euclidean2D>> boundaries = new ArrayList<SubHyperplane<Euclidean2D>>();
        boundaries.add(line1.wholeHyperplane());
        boundaries.add(line2.wholeHyperplane());
        boundaries.add(buildSegment(new Vector2D(0, 3), new Vector2D(0, -3)));
    ...
    public void testMixedFiniteAndInfiniteLines_impliedInfiniteBoundaries() {
        // arrange
        Line line = new Line(new Vector2D(3, 0), new Vector2D(3, 3), TEST_TOLERANCE);

        List<SubHyperplane<Euclidean2D>> boundaries = new ArrayList<SubHyperplane<Euclidean2D>>();
        boundaries.add(buildSegment(new Vector2D(0, 3), new Vector2D(0, 0)));
        boundaries.add(buildSegment(new Vector2D(0, 0), new Vector2D(3, 0)));
        ...
        checkVertexLoopsEquivalent(new Vector2D[][] {
            {
                null,
                new Vector2D(0, 1), // dummy point
                new Vector2D(0, 0),
                new Vector2D(3, 0),
                new Vector2D(3, 1) // dummy point
            }
        }, poly.getVertices());

        checkPoints(Region.Location.INSIDE, poly,
                new Vector2D(0.1, Float.MAX_VALUE),
                new Vector2D(0.1, 0.1),
                new Vector2D(1.5, 0.1),
                new Vector2D(2.9, 0.1),
                new Vector2D(2.9, Float.MAX_VALUE));
        checkPoints(Region.Location.OUTSIDE, poly,
                new Vector2D(-0.1, Float.MAX_VALUE),
                new Vector2D(-0.1, 0.1),
                new Vector2D(1.5, -0.1),
                new Vector2D(3.1, 0.1),
                new Vector2D(3.1, Float.MAX_VALUE));
        checkPoints(Region.Location.BOUNDARY, poly,
                new Vector2D(0, 1),
                new Vector2D(1, 0),
                new Vector2D(3, 1));
    ...
    public void testBox() {
        ...
        GeometryTestUtils.assertVectorEquals(new Vector2D(1, 0), (Vector2D) box.getBarycenter(), TEST_TOLERANCE);
    ...
    public void testInvertedBox() {
        // arrange
        List<SubHyperplane<Euclidean2D>> boundaries = new ArrayList<SubHyperplane<Euclidean2D>>();
        boundaries.add(buildSegment(new Vector2D(0, -1), new Vector2D(0, 1)));
        boundaries.add(buildSegment(new Vector2D(2, 1), new Vector2D(2, -1)));
        boundaries.add(buildSegment(new Vector2D(0, 1), new Vector2D(2, 1)));
        boundaries.add(buildSegment(new Vector2D(2, -1), new Vector2D(0, -1)));
        ...
        Assert.assertFalse(box.isEmpty());
        Assert.assertFalse(box.isFull());
        GeometryTestUtils.assertVectorEquals(Vector2D.NaN, (Vector2D) box.getBarycenter(), TEST_TOLERANCE);
    ...
    public void testHole() {
        ...
        for (double x = -0.999; x < 3.999; x += 0.11) {
            ...
            if (x < -0.5) {
                ...
                Assert.assertEquals(3.0,      p.getY(), TEST_TOLERANCE);
                Assert.assertEquals(+v.distance(new Vector2D(3, 3)), projection.getOffset(), TEST_TOLERANCE);
            }
        }
    }
    ...
    public void testLineIntersection() {
        ...
        // assert
        Line l1 = new Line(new Vector2D(-1.5, 0.0), FastMath.PI / 4, TEST_TOLERANCE);
        ...
        Line l2 = new Line(new Vector2D(-1.0, 2.0), 0, TEST_TOLERANCE);
    ...
    public void testChoppedHexagon() {
        ...
        SubLine[] hyp = {
            new Line(new Vector2D(   0.0, 1.0),  5 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(-sqrt3, 1.0),  7 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(-sqrt3, 1.0),  9 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(-sqrt3, 0.0), 11 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(   0.0, 0.0), 13 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(   0.0, 1.0),  3 * pi6, TEST_TOLERANCE).wholeHyperplane(),
            new Line(new Vector2D(-5.0 * sqrt3 / 6.0, 0.0), 9 * pi6, TEST_TOLERANCE).wholeHyperplane()
        };
        ...
        SubLine splitter =
            new Line(new Vector2D(-2.0 * sqrt3 / 3.0, 0.0), 9 * pi6, TEST_TOLERANCE).wholeHyperplane();
    ...
    public void testBug20041003() {
        ...
        Line[] l = {
            new Line(new Vector2D(0.0, 0.625000007541172),
                     new Vector2D(1.0, 0.625000007541172), TEST_TOLERANCE),
            new Line(new Vector2D(-0.19204433621902645, 0.0),
                     new Vector2D(-0.19204433621902645, 1.0), TEST_TOLERANCE),
            new Line(new Vector2D(-0.40303524786887,  0.4248364535319128),
                     new Vector2D(-1.12851149797877, -0.2634107480798909), TEST_TOLERANCE),
            new Line(new Vector2D(0.0, 2.0),
                     new Vector2D(1.0, 2.0), TEST_TOLERANCE)
        };
