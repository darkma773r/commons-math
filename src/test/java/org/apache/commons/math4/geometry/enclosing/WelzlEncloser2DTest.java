...
public class WelzlEncloser2DTest {
    ...
    public void testNullList() {
        ...
        WelzlEncloser<Euclidean2D, Point2D> encloser =
                new WelzlEncloser<>(1.0e-10, generator);
        EnclosingBall<Euclidean2D, Point2D> ball = encloser.enclose(null);
    ...
    public void testNoPoints() {
        ...
        WelzlEncloser<Euclidean2D, Point2D> encloser =
                new WelzlEncloser<>(1.0e-10, generator);
        EnclosingBall<Euclidean2D, Point2D> ball = encloser.enclose(new ArrayList<Point2D>());
        Assert.assertTrue(ball.getRadius() < 0);
    }
    ...
    private void checkDisk(List<Point2D> points, List<Point2D> refSupport) {
        ...
        EnclosingBall<Euclidean2D, Point2D> disk = checkDisk(points);

        // compare computed disk with expected disk
        DiskGenerator generator = new DiskGenerator();
        EnclosingBall<Euclidean2D, Point2D> expected = generator.ballOnSupport(refSupport);
        ...
        for (int i = 0; i < disk.getSupportSize(); ++i) {
            ...
            EnclosingBall<Euclidean2D, Point2D> reducedDisk = generator.ballOnSupport(reducedSupport);
    ...
    private EnclosingBall<Euclidean2D, Point2D> checkDisk(List<Point2D> points) {

        WelzlEncloser<Euclidean2D, Point2D> encloser =
                new WelzlEncloser<>(1.0e-10, new DiskGenerator());
        EnclosingBall<Euclidean2D, Point2D> disk = encloser.enclose(points);
