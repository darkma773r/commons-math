...
public class DiskGeneratorTest {
    ...
    public void testSupport0Point() {
        ...
        EnclosingBall<Euclidean2D, Point2D> disk = new DiskGenerator().ballOnSupport(support);
    ...
    public void testRandom() {
        ...
        for (int i = 0; i < 500; ++i) {
            ...
            Point2D refCenter = new Point2D(d, new Point2D(sr.nextVector()));
            List<Point2D> support = new ArrayList<>();
            for (int j = 0; j < 3; ++j) {
                support.add(new Point2D(1.0, refCenter, refRadius, new Point2D(sr.nextVector())));
            }
            EnclosingBall<Euclidean2D, Point2D> disk = new DiskGenerator().ballOnSupport(support);
